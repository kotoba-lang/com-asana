(ns asana.main
  "Kotodama WASM entrypoint for the Asana clean-room actor (L4) — Clojure port.
  ...
  facts (`asana.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Task"      :plural "tasks"      :id-prefix "asana_tas"
    :fields [:name :resourceSubtype :completed :completedAt :approvalStatus :dueAt] :required [:name :resourceSubtype]
    :coerce {:completed :bool}          :refs {}}
   {:entity "Project"   :plural "projects"   :id-prefix "asana_pro"
    :fields [:name :archived :color :completed :defaultView]            :required [:name :archived]
    :coerce {:archived :bool :completed :bool}       :refs {}}
   {:entity "Section"   :plural "sections"   :id-prefix "asana_sec"
    :fields [:name :createdAt]                       :required [:name :createdAt]
    :coerce {}                       :refs {}}
   {:entity "User"      :plural "users"      :id-prefix "asana_use"
    :fields [:name :email]                           :required [:name :email]
    :coerce {}                       :refs {}}
   {:entity "Tag"       :plural "tags"       :id-prefix "asana_tag"
    :fields [:name :color :notes]                    :required [:name :color]
    :coerce {}                       :refs {}}
   {:entity "Workspace" :plural "workspaces" :id-prefix "asana_wor"
    :fields [:name :isOrganization]                  :required [:name :isOrganization]
    :coerce {:isOrganization :bool}   :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"asana.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "asana-compat" :tier tier :entities entities} 200])
